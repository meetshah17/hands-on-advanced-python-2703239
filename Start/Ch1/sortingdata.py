...
sorted_dataset = sorted(dataset, key=lambda d: d['snow']>0.0,reverse=True)
# pprint.pp(sorted_dataset)

# TODO: method 2: use the sort() function that every list has to sort in-place
dataset.sort(key = lambda d:d['snow']>0.0, reverse = True)

# TODO: Sort on multiple fields: first by snowfall, then by average wind speed
sorted_dataset = sorted(dataset, key=lambda d: (d['snow'],d['awnd']),reverse=False)
pprint.pp(sorted_dataset)
